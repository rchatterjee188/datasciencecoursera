...
currentTime <- Sys.time()# time by POSIXct(large integer vector, useful in dataframe) or POSIXlt(list, other info stored like day) class, stored internally as number of seconds since 1970-01-01
...
Sys.time() # get current time as POSIXct
...
x[1]-x[2] # time difference, track of discrepancies (i.e. daylightsavings, timezones, leap time, etc)
...
5+7 # basic arithmetic operations all work +, -, *, /, ^. NA in expression returns NA.
sqrt(4) # square root
abs(-1:2) # absolute value
# Logical operators
5 >= 2 # returns logical. <, >, <=, >=, ==, !=. NA in expression returns NA.
TRUE | FALSE # OR A|B union, AND A&B intersection, NOT !A negation
# Character functions
...
y <- if(x>3){ # testing condition
  10
} else if(x > 0) { # can not have or multiple
  5
